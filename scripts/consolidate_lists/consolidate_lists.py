...
def load_facilities(xlsx_path: Path) -> pd.DataFrame:
    ...
    out["_street_norm"] = out.get("adresse_strasse", "").apply(norm_street)
    out["_addr_key"] = out.apply(lambda r: make_addr_key(r["_zip"], r.get("adresse_ort",""), r.get("adresse_strasse","")), axis=1)
    return out
...
def load_legal(xlsx_path: Path) -> pd.DataFrame:
    ...
    out["_city_norm"] = out.get("Ort", "").apply(norm_city)
    out["_street_norm"] = out.get("Straße + Hausnr.", "").apply(norm_street)
    out["_addr_key"] = out.apply(lambda r: make_addr_key(r["_zip"], r.get("Ort",""), r.get("Straße + Hausnr.","")), axis=1)
    return out
...
def load_domains(path: Path) -> pd.DataFrame:
    ...
    out["_zip"] = out.get("plz", "").apply(norm_zip)
    out["_city_norm"] = out.get("ort", "").apply(norm_city)
    out["_street_norm"] = street_disp.apply(norm_street)
    out["_addr_key"] = out.apply(lambda r: make_addr_key(r["_zip"], r.get("ort",""), (str(r.get("strasse","")) + " " + str(r.get("hausnummer","")))), axis=1)
    return out
